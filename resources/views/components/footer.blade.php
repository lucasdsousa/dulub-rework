@include('layouts.app')

<style>         
    hr {
        color: white;
        border: solid 1px white;
        border-radius: 100px;
        width: 1000px;
        margin-right: auto;
        margin-left: auto;
}
</style>

<div class="container-fluid" style="background-color: black">
    <div class="row">
        <div class="col mt-3" style="margin-left:100px">
            <img src="images/Logo - Dulub Branca.png" style="width:40%; margin-left:-25px" alt="">

            <p style="color: white">Empresa genuinamente nordestina, com atuação integralizada e sempre crescente no mercado de 
                óleos automotivos, industrias e graxas.</p>

            <br>

            <p style="color: white">DUNAX LUBRIFICANTES LTDA <br> 
                                    Núcleo CIS, Lote 02, Quadra 03 - S/N - São Gonçalo dos Campos/BA CEP 44.330-000 <br>
                                    dulub@dulub.com.br <br>
                                    (85) 3275.3070 | 0800 730 30 70</p>
        </div>
        
        <div class="col mt-5" style="margin-left:150px">
            <h5 style="color: white"><strong>Links Principais</strong></h5>
            <a href="#" style="color: white">Produtos</a> <br>
            <a href="#" style="color: white">Catálogo</a> <br>
            <a href="#" style="color: white">Blog</a> <br>
            <a href="#" style="color: white">Parceiros</a> <br>
        </div>
        
        <div class="col mt-5">
            <h5 style="color: white"><strong>Links Úteis</strong></h5>
            <a href="#" style="color: white">Sobre Nós</a> <br>
            <a href="#" style="color: white">Trabalhe Conosco</a> <br>
            <a href="#" style="color: white">Política de privacidade</a> <br>
        </div>

        <div class="container mt-5">
            <hr>
            <p style="text-align:center; font-size: 12px; color:white">&copy 2022 Todos os direitos reservados.</p>
        </div>

    </div>
    
</div>